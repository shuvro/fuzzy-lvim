...
augroup END
  ...
  create_aucmd("TermOpen", {
    group = "_lvim_user",
    pattern = "term://*",
    command = "lua require('lvim.user.keybindings').set_terminal_keymaps()",
  })
  if lvim.builtin.metals.active then
    create_aucmd("Filetype", {
      group = "_lvim_user",
      pattern = { "scala", "sbt" },
      callback = require("lvim.user.metals").start,
    })
  end
  create_aucmd("FileType", {
    group = "_lvim_user",
  ...
  local codelens_viewer = "lua require('lvim.user.codelens').show_line_sign()"
